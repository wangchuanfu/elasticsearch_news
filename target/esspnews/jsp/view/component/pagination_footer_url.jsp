...
<!--
	...
	function toPage(){
		...
		window.location.href="${pageContext.request.contextPath}"+$("#urlParam").val()+p+".html"; 
	}
	
	function clickPageNum2(arg,pageNum)
	{		
		window.location.href=arg+pageNum+".html"; 
	}
//-->
</script>


<input type="hidden" id="urlParam" value="${pagination.requestURL}" />
...
<div style="width:450px; float:right;">共${pagination.totalCount }条:						
	...
	<c:if test="${pagination.totalPage > 1 and pagination.totalPage < 7}">
		...
		<c:if test="${pagination.page != 1}">
			<span>
				<a onclick="clickPageNum2('<%=WebURLParams.LIST_PAGE_URL %>${pagination.requestURL}','1')" href="javascript:void(0)" title="首页">首页</a>
			</span>
			<span>
				<a onclick="clickPageNum2('<%=WebURLParams.LIST_PAGE_URL %>${pagination.requestURL}','${pagination.page-1}')" href="javascript:void(0)" title="上一页">上一页</a>
			</span>
		</c:if>
		<c:forEach var="i" begin="1" end="${pagination.totalPage}">
			...
			<c:if test="${pagination.page ne i}">
				<span>
					<a onclick="clickPageNum2('<%=WebURLParams.LIST_PAGE_URL %>${pagination.requestURL}','${i}')" href="javascript:void(0)">${i}</a>
				</span>
			</c:if>		
		</c:forEach>
		<c:if test="${pagination.page != pagination.totalPage}">
			<span>
				<a onclick="clickPageNum2('<%=WebURLParams.LIST_PAGE_URL %>${pagination.requestURL}','${pagination.page+1}')" href="javascript:void(0)" title="下一页">下一页</a>
			</span>
			<a onclick="clickPageNum2('<%=WebURLParams.LIST_PAGE_URL %>${pagination.requestURL}','${pagination.totalPage}')" href="javascript:void(0)" title="末页">末页</a>
		</c:if>
