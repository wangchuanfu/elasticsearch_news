...
<!--
	function toPage(){
		...
		window.location.href = "<%=WebURLParams.SEARCH_PAGE_URL %>/s${pagination.requestURL}"+p+"-${searchType }.html";
		return false;
	}
//-->
</script>
...
共${pagination.totalCount }条: 							
	...
	<c:if test="${pagination.totalPage > 1 and pagination.totalPage <= 7}">
		...
		<c:if test="${pagination.page != 1}">
			<span>
				<a href="<%=WebURLParams.SEARCH_PAGE_URL %>/s${pagination.requestURL}1-${searchType }.html" title="首页">首页</a>
			</span>
			<span>
				<a href="<%=WebURLParams.SEARCH_PAGE_URL %>/s${pagination.requestURL}${pagination.page-1}-${searchType }.html" title="上一页">上一页</a>
			</span>
		</c:if>
		<c:forEach var="i" begin="1" end="${pagination.totalPage}">
			...
			<c:if test="${pagination.page ne i}">
				<span>
					<a href="<%=WebURLParams.SEARCH_PAGE_URL %>/s${pagination.requestURL}${i}-${searchType }.html">${i}</a>
				</span>
			</c:if>		
		</c:forEach>
		<c:if test="${pagination.page != pagination.totalPage}">
			<span>
				<a href="<%=WebURLParams.SEARCH_PAGE_URL %>/s${pagination.requestURL}${pagination.page+1}-${searchType }.html" title="下一页">下一页</a>
			</span>
			<a href="<%=WebURLParams.SEARCH_PAGE_URL %>/s${pagination.requestURL}${pagination.totalPage}-${searchType }.html" title="末页">末页</a>
		</c:if>
