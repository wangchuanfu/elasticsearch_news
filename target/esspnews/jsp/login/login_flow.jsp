...
<%@page import="com.huayuan.search.utils.XSSUtils"%>
...
<script type="text/javascript" src="<%=WebURLParams.CJ_URL %>/js/login/login_flow.js?20140723"></script>
<link type="text/css" rel="stylesheet" href="<%=WebURLParams.CJ_URL %>/css/common/emailAuto.css" />
<link rel="stylesheet" href="<%=WebURLParams.CJ_URL %>/css/minilogin.css?20140723" />
<style>
.mini-register {
	
}
</style>
<!-- <div class="mini-bg" id="newmini-bg" style="display:none;"></div> -->
<!-- <div id="miniCC">
</div> -->
<div class="mini-login" id="newover_login" style="display: none;">
	<div class="mn-tit">
		<span>您尚未登录</span><a href="javascript:;" id="newbtn_close" onclick="document.getElementById('newover_login').style.display='none';">close</a>
	</div>
	<div class="mn-tab">
		<ul>
			<li class="act">登录</li>
	...
	<div class="mn-con">
		<!-- 登录 -->
		<form class="mndl" name="newloginForm" method="post" action="login">
			<div>
				<!-- 账号 -->
				<div class="item minicj02">
					<label class="lable">账号</label>
					<div class="item-sj">
						<div class="o-regName">
							<!--获取焦点 bian-01为输入边框 bian-02为报错边框 -->
							<input type="text" onblur="if(this.value=='') {this.value='请输入邮箱/手机号/用户名';this.style.color='#999999'}" onfocus="onfocusMiniUserName()" value="请输入邮箱/手机号/用户名" onpaste="return false;" autocomplete="off" class="text bian-01" style="color: #999;" id="userName" name="userName" />

							<!-- error报错 -->
							<em id="miniLoginName"></em>
							<!-- 报错信息 -->
							<div class="er-tit" id="miniLoginNameDiv"></div>
						</div>
					</div>
				</div>
				<!-- 密码 -->
				<div class="item">
					...
					<div class="item-sj">
						<div class="o-regName">
							<!--获取焦点 bian-01为输入边框 bian-02为报错边框 -->
							<input type="password" class="text bian-01" id="password" name="password" onfocus="onfocusMiniPwd()" />
							<!-- error报错 -->
							<em id="miniPassword"></em>
							<!-- 报错信息 -->
							<div class="er-tit" id="miniPasswordDiv"></div>
						</div>
					</div>
				</div>
				<!-- 自动登录 -->
				<div class="item jzzh">
					<div class="fl">
						<input type="checkbox" name="miniRememberUser" id="miniRememberUser" class="checkbox" value="1" /> <label class="mar">自动登录</label>
					</div>
					<div class="fr">
						<a href="<%=WebURLParams.HOME_PAGE_URL %>/show-forgetPassword.html">忘记密码?</a>
					</div>
				</div>
				<!-- 登录 -->
				<div class="item">
					<input type="button" onclick="beginlogin();" class="zc-ljzc" value="登录" />
				</div>
				<!-- 合作伙伴 -->
				<div class="hzwz clearfix">
					<p>使用合作网站账号登录健一网：</p>
					<a href="javascript:void(0);" class="hzico ico-01" onclick="openQQLoginWindow();">QQ</a> <a href="javascript:void(0);" class="hzico ico-02" onclick="openAlipayLoginWindow();">支付宝</a> <a href="javascript:void(0);" class="hzico ico-03" onclick="openVincceWindow()"></a>
				</div>
			</div>
		</form>
		<!-- 注册 -->
		<form style="display: none" class="mndl" name="newMemberForm" method="post" action="<%=WebURLParams.HOME_PAGE_URL %>/member-newRegister.html?mini=mini&minibackURL=<%=null==request.getParameter("backURL")?"":XSSUtils.protectAgainstXSS(request.getParameter("backURL"))%>" onsubmit="return checkMiniRegister();">
			<input type="hidden" id="minibackURL" name="minibackURL" value="<%=null == request.getParameter("backURL") ? "" : XSSUtils
					.protectAgainstXSS(request.getParameter("backURL"))%>" />
					 <input type="hidden" id="newflowName" name="newflowName" value="${sessionScope.loginName }" /> <input id="mobileState" name="mobileState" value="N" type="hidden">
			<div>
				<!-- 账号 -->
				<div class="item minicj02">
					<label class="lable">账号</label>
					<div class="item-sj">
						<div class="o-regName">
							<!--获取焦点 bian-01为输入边框 bian-02为报错边框 -->
							<div class="parentCls">
								<input type="text" onblur="checkMiniUser()" onfocus="checkMminiUserOnFocus()" value="请输入邮箱/手机号" onpaste="return false;" autocomplete="off" class="text bian-01 mini-register" name="miniRegisterName" id="miniRegisterName" style="color: #999;" />
							</div>
							<!-- error correct报错和正确图标 class="error"  -->
							<em id="miniErrorRegisterName"></em> <input id="ministoreemail" name="ministoreemail" value="" type="hidden">
							<!-- 报错信息 -->
							<div class="er-tit" id="miniErrorRegisterNameDiv"></div>
						</div>
					</div>
				</div>
				<!-- 密码 -->
				<div class="item">
					...
					<div class="item-sj">
						<div class="o-regName">
							<!--获取焦点 bian-01为输入边框 bian-02为报错边框 -->
							<input type="password" class="text bian-01" id="miniPwd" name="miniPwd" onfocus="checkMiniPwdOnfocus()" onblur="checkMiniPwd()" />
							<!-- error correct报错和正确图标 -->
							<em id="miniErrorRegisterPwd"></em>
							<!-- 报错信息 -->
							<div class="er-tit" id="miniErrorRegisterPwdDiv"></div>
						</div>
					</div>
					...
					<div class="item-sj">
						<div class="o-regName">
							<!--获取焦点 bian-01为输入边框 bian-02为报错边框 -->
							<input type="password" class="text bian-01" id="reminiRegisterPwd" name="reminiRegisterPwd" onfocus="checkMiniRePwdOnfocus()" onblur="checkMiniRePwd()" />
							<!-- error correct报错和正确图标 -->
							<em id="reminiErrorRegisterPwd"></em>
							<!-- 报错信息 -->
							<div class="er-tit" id="reminiErrorRegisterPwdDiv"></div>
						</div>
					</div>
				</div>
				<!-- 邮箱注册时普通验证码 -->
				<div class="item clearfix" id="miniptyzm">
					<label class="lable">验证码</label>
					<div class="fl item-sj">
						<div class="o-regName">
							<!--获取焦点 bian-01为输入边框 bian-02为报错边框 -->
							<input type="text" class="text text-1  bian-01" id="miniptymvalue" name="miniptymvalue" onblur="checkMiniYzm('pt')" onfocus="focusYzm('pt')" />
							<!-- 邮箱验证 -->
							<label class="img"><img src="<%=WebURLParams.HOME_PAGE_URL %>/system/keyword.html" alt="" id="miniymimg" /></label> <label class="kbq"><a href="javascript:void(0);" onclick="refreshMiniRegisterCode();return false;" id="minikbqhyz">看不清？换一张</a> </label>

							<!-- class="error yanzhen" error correct报错和正确图标 -->
							<em id="minilabelym" class="yanzhen"></em>
							<!-- 报错信息 -->
							<div class="er-tit" id="minilabelymDiv"></div>
						</div>
					</div>
				</div>

				<!--手机注册时显示的手机短信验证码  -->
				<div class="item clearfix" id="minimobilyzm" style="display: none;">
					<label class="lable">验证码</label>
					<div class="fl item-sj">
						<div class="o-regName">
							<!--获取焦点 bian-01为输入边框 bian-02为报错边框 -->
							<input type="text" class="text text-1 bian-01" id="minimobilymvalue" name="minimobilymvalue" onblur="checkMiniYzm('mobil')" onfocus="focusYzm('mobil')" />
							<!-- 手机获取验证码 -->
							<a href="javascript:;" class="hqyzm" id="getMiniCode" onclick="getMiniMobileCode('miniRegisterName','minilabelmobil','getMiniCode')">获取短信验证码</a>
<!-- 								<input type="button" class="hqyzm" value="获取短信验证码" id="getMiniCode" onclick="getMiniMobileCode('miniRegisterName','minilabelmobil','getMiniCode')" style="cursor: pointer;"/> -->
							<!--  class="error yanzhen" error correct报错和正确图标 -->
							<em id="minilabelmobil" class="yanzhen"></em>
							<!-- 报错信息 -->
							<div class="er-tit" id="minilabelmobilDiv"></div>
						</div>
					</div>
				</div>
				<!-- 协议 -->
				<div class="item-xy">
					<input type="checkbox" checked="checked" class="checkbox" id="protocolMini" value="1"> <label for="protocol">我已阅读并同意<a class="red" href="#">《健一网用户协议》</a></label>
					<!--错误提示信息  -->
					<label for="" style="color: red;" id="labelMiniProtocol"></label>
				</div>
				<!-- 登录 -->
				<div class="item">
					<input type="submit" class="zc-ljzc zc-mo" value="注册" />
				</div>
			</div>
		</form>
	</div>
...
<script>
	$(function() {
		$(".mn-tit a").click(function() {
			$(".mini-bg").hide();
			$(".mini-login").hide();
		});
		$(".mn-tab li").click(function() {
			$(this).addClass('act').siblings().removeClass('act');
			var index = $(this).index();
			$(".mn-con .mndl").eq(index).show().siblings().hide();
		});
	})
</script>
...
<script type="text/javascript">
	...
	function EmailAutoComplete(options) {
		...
		this.config = {
			targetCls : '.mini-register', // 目标input元素
