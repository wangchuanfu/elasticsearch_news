				...
				<dd>
					<c:forEach var="selectPara" items="${paraList }" varStatus="i">
						<c:if test="${selectPara.endUrl != ''}">
							<a href="<%=WebURLParams.LIST_PAGE_URL %>/p-${selectPara.beginUrl}0${selectPara.endUrl }.html"><h5>${selectPara.attrName }：</h5>${fn:split(selectPara.attrs, '_')[1] }
						 	<i></i></a>
						</c:if>
						<c:if test="${selectPara.endUrl == ''}">
							<a href="<%=WebURLParams.LIST_PAGE_URL %>/p-${selectPara.beginUrl}${selectPara.endUrl }.html"><h5>${selectPara.attrName }：</h5>${fn:split(selectPara.attrs, '_')[1] }
						 	<i></i></a>
						</c:if>
					</c:forEach>
					<a href="<%=WebURLParams.LIST_PAGE_URL %>/p-${catalogId}.html" class="rechose">重新筛选</a>
				</dd>
			</dl>
			...
			<dl class="clearfix choosetype">
				...
				<dd id="pl_brand${i.index }">
					<c:forEach var="att" items="${fn:split(attr.attrs, ',')}">
						<c:if test="${att != '' }">
							<a href="<%=WebURLParams.LIST_PAGE_URL %>/p-${attr.beginUrl}${fn:split(att, '_')[0]}${attr.endUrl}.html">${fn:split(att, '_')[1]}</a>
						</c:if>
					</c:forEach>
