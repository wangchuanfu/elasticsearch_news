...
<!--  注释筛选条件 -->
 <div class="j1131025_searchlist">
		<h1> <em>${catalog.proCatalogName }</em></h1>
			<dl class="choose clearfix">
				<dt>您已经选择：</dt>
				<dd>
					<c:forEach var="selectPara" items="${paraList }" varStatus="i">
						 <c:if test="${selectPara.endUrl != ''}">
							<a href="<%=WebURLParams.LIST_PAGE_URL %>/plistSearch/${selectPara.endUrl }/1-${catalogId}-0-1.html"><h5>${selectPara.attrName }：</h5>${fn:split(selectPara.attrs, '_')[1] }
						 	<i></i></a>
						</c:if>
						<c:if test="${selectPara.endUrl == ''}">
							<a href="<%=WebURLParams.LIST_PAGE_URL %>/plistSearch/${selectPara.endUrl }/1-${catalogId}-0-1.html"><h5>${selectPara.attrName }：</h5>${fn:split(selectPara.attrs, '_')[1] }
					...
					<a href="<%=WebURLParams.LIST_PAGE_URL %>/plistSearch/1/1-${catalogId}-0-1.html" class="rechose">重新筛选</a>
				</dd>
			</dl>
			...
			<dl class="clearfix choosetype">
				...
				<dd id="pl_brand${i.index }">
					<c:forEach var="att" items="${fn:split(attr.attrs, ',')}">
						<c:if test="${att != '' }">
							<a href="<%=WebURLParams.LIST_PAGE_URL %>/plistSearch/${attr.endUrl}${fn:split(att, '_')[0]}/1-${catalogId}-0-1.html">${fn:split(att, '_')[1]}</a>
						</c:if>
					</c:forEach>
